               ...
               mediaListSet.then(function() {
                                 activeUser.getStoredFilesListAsync("podcasts", mediaInit, function(foundPodcasts) {
                                                                    ...
                                                                    msgTitle = resources.connError;
                                                                        ...
                                                                        $("#addbutton").html("<div id='getStoredFilesListAsync130' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                                                        modal.style.display = "block";
                                                                        $(document).off("vclick", "#getStoredFilesListAsync130");
                                                                        $(document).on("vclick", "#getStoredFilesListAsync130", function (event) {
...
function playPodcastTheory(podcastItem) {
    try {
        ...
        if (podcastItem.isdownloaded === false && deviceIsOnline === false) {
            ...
            $("#boxTitle").html(msgTitle);
            $("#box-string").html(msgStr);
            $("#addbutton").html("<div id='playPodcastTheory324' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

            modal.style.display = "block";
            $(document).off("vclick", "#playPodcastTheory324");
            $(document).on("vclick", "#playPodcastTheory324", function (event) {
                modal.style.display = "none";
            });
            //navigator.notification.confirm(msgStr, function() {}, msgTitle, msgBtnValue);
        } else {
...
function podcastPlayNext(podplayer, playertype) {
    try {
        ...
        if (currentPlayListItem.isdownloaded === false && deviceIsOnline === false) {
            ...
            $("#boxTitle").html(msgTitle);
            $("#box-string").html(msgStr);
            $("#addbutton").html("<div id='podcastPlayNext' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

            modal.style.display = "block";
            $(document).off("vclick", "#podcastPlayNext");
            $(document).on("vclick", "#podcastPlayNext", function (event) {
                modal.style.display = "none";
            });
           // navigator.notification.confirm(msgStr, function() {}, msgTitle, msgBtnValue);
        } else {
