...
init: function() {
    try {
        ...
        this.coursesDB.transaction(function(tx) {
            tx.executeSql("CREATE TABLE IF NOT EXISTS ETEACHER_COURSE (id unique, courseid, title, status, coursestatusid,buypackage,freeaccess,coursecost,allowextension,coursediscounttype,coursediscountvalue,maxextensionterm,validfor,minimumextensionfee,purchasedtapid,isBuypackageavailable,allowextension,PaymentMethodId,PaymentOptionId)");
                                   });
    ...
    try {
        this.coursesDB.transaction(function(tx) {
          tx.executeSql("SELECT * FROM ETEACHER_COURSE", [], function(tx, results) {
            ...
            for (var i = 0; i < len; i++) {
                var savedCourse = new Course(results.rows.item(i).courseid, results.rows.item(i).title, results.rows.item(i).status, results.rows.item(i).coursestatusid, results.rows.item(i).buypackage, results.rows.item(i).freeaccess, results.rows.item(i).coursecost, results.rows.item(i).allowextension, results.rows.item(i).coursediscounttype, results.rows.item(i).coursediscountvalue, results.rows.item(i).maxextensionterm, results.rows.item(i).validfor, results.rows.item(i).minimumextensionfee, results.rows.item(i).purchasedtapid, results.rows.item(i).isBuypackageavailable, results.rows.item(i).PaymentMethodId, results.rows.item(i).PaymentOptionId);
    ...
    try {
        ...
        coController.coGetCourses(function(ret) {
                                  ...
                                  msgBtnValue = resources.btnOk;
                                      ...
                                      $("#addbutton").html("<div id='refreshCourses54' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                      modal.style.display = "block";
                                      $(document).off("vclick", "#refreshCourses54");
                                      $(document).on("vclick", "#refreshCourses54", function (event) {
                                  ...
                                  if (usercourses.length == undefined) {
                                      ...
                                      course = new Course(courseobj.CourseId, courseobj.Title, courseobj.Status, courseobj.CourseStatusId, courseobj.Display, courseobj.Accessible, courseobj.InAccessibleFunction, courseobj.BuyPackage, courseobj.FreeAccess, courseobj.CourseCost, courseobj.AllowExtension, courseobj.CourseDiscountType, courseobj.CourseDiscountValue, courseobj.MaxExtensionTerm, courseobj.ValidFor, courseobj.MinimumExtensionFee, courseobj.PurchasedTapId, courseobj.IsBuyPackageAvailable, courseobj.PaymentMethodId, courseobj.PaymentOptionId);
            ...
            $.each(this.courses, function(i, course) {
                ...
                var coursesingleline = "";
                   ...
                   if (course.accessible) {
                       ...
                       courseStr += "<h3 id='h3-" + courseid +"'  onclick='assessmentPackage();' class='divider clientheader'><div class='coursetitlediv'><div id='cert" + courseid + "' class='certificateicon";
...
save: function(course) {
    try {
        ...
        if (course.coursecost == undefined) {
            course.coursecost = "";
        }
        if (!course.allowextension) {
            course.allowextension = "allowextensionFalse";
        }
        if (course.coursediscounttype == undefined) {
            course.coursediscounttype = "";
        }
        if (course.coursediscountvalue == undefined) {
            course.coursediscountvalue = "";
        }
        if (course.maxextensionterm == undefined) {
            course.maxextensionterm = "";
        }
        if (course.validfor == undefined) {
            course.validfor = "";
        }
        if (course.PaymentMethodId == undefined) {
            course.PaymentMethodId = "";
        }
        if (course.PaymentOptionId == undefined) {
            course.PaymentOptionId = "";
        }
        if (course.minimumextensionfee == undefined) {
            course.minimumextensionfee = "";
        }
        ...
        this.usersDB.transaction(function(tx) {
            ...
            tx.executeSql("INSERT INTO ETEACHER_COURSE(courseid, title, status, coursestatusid,buypackage,freeaccess,coursecost,allowextension,coursediscounttype,coursediscountvalue,maxextensionterm,validfor,minimumextensionfee,purchasedtapid,isBuypackageavailable,PaymentMethodId,PaymentOptionId) VALUES (course.courseid, course.title, course.status, course.coursestatusid,course.buypackage,course.freeaccess,course.coursecost,course.allowextension,course.coursediscounttype,course.coursediscountvalue,course.maxextensionterm,course.validfor,course.minimumextensionfee,course.purchasedtapid,course.isBuypackageavailable,course.PaymentMethodId,course.PaymentOptionId)");
                                 });
    } catch (e) {
...
var Course = function (courseid, title, status, coursestatusid, display, accessible, inaccessiblefunction, buypackage, freeaccess, coursecost, allowextension, coursediscounttype, coursediscountvalue, maxextensionterm, validfor, minimumextensionfee, purchasedtapid, isBuypackageavailable, PaymentMethodId, PaymentOptionId) {
    try {
        this.init(courseid, title, status, coursestatusid, display, accessible, inaccessiblefunction, buypackage, freeaccess, coursecost, allowextension, coursediscounttype, coursediscountvalue, maxextensionterm, validfor, minimumextensionfee, purchasedtapid, isBuypackageavailable, PaymentMethodId, PaymentOptionId);
    } catch (e) {
        errorHandler("Course", e);
    }
};
Course.prototype = {
    init: function (courseid, title, status, coursestatusid, display, accessible, inaccessiblefunction, buypackage, freeaccess, coursecost, allowextension, coursediscounttype, coursediscountvalue, maxextensionterm, validfor, minimumextensionfee, purchasedtapid, isBuypackageavailable, PaymentMethodId, PaymentOptionId) {
    try {
        this.modulegroups = [];
        ...
        if (coursecost == undefined) {
            coursecost = 0;
        }
        if (!allowextension) {
            allowextension = "allowextensionFalse";
        }
        if (coursediscounttype == undefined) {
            coursediscounttype = "";
        }
        if (coursediscountvalue == undefined) {
            coursediscountvalue = 0;
        }
        if (maxextensionterm == undefined) {
            maxextensionterm = 0;
        }
        ...
        if (minimumextensionfee == undefined) {
            minimumextensionfee = 0;
        }
        ...
        this.minimumextensionfee = minimumextensionfee;
    ...
    try {
        ...
        if (inaccessiblefunction.indexOf("prevModulesIncomplete") > -1 && status == "TAPRequired") {
            ...
            if (inaccessiblefunction.indexOf("modulePaymentRequired") > -1 && status != "TAPRequired") {
                ...
                inaccessiblefunction = "TAPRquired";
            }
            else {
    ...
    try {
        ...
        coController.coGetLessonStructure(this.basemoduleid,this.courseid, function(ret) {
                                          ...
                                          msgBtnValue = resources.btnOk;
                                              ...
                                              $("#boxTitle").html(msgTitle);
                                              $("#box-string").html(msgStr);
                                              $("#addbutton").html("<div id='refreshLessons1332' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                              modal.style.display = "block";
                                              $(document).off("vclick", "#refreshLessons1332");
                                              $(document).on("vclick", "#refreshLessons1332", function (event) {
                                                  modal.style.display = "none";
                                                  hidePleaseWait();
                                                  returnFunction(false);
                                              });
                                          ...
                                          that.theoryModule = new TheoryModule(that.courseid, that.basemoduleid, theoryModObj.HasPodcast, theoryModObj.PDFUrl, theoryModObj.PodcastUrl, theoryModObj.LSURLIsYUDU, theoryModObj.YUDUID, theoryModObj.BuyPackage);
    ...
    try {
        ...
        coController.coGetGuidelines(this.courseid, this.basemoduleid, function(ret) {
                                     ...
                                     msgBtnValue = resources.btnOk;
                                         ...
                                         $("#boxTitle").html(msgTitle);
                                         $("#box-string").html(msgStr);
                                         $("#addbutton").html("<div id='setGuidelineViews1784' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                         modal.style.display = "block";
                                         $(document).off("vclick", "#setGuidelineViews1784");
                                         $(document).on("vclick", "#setGuidelineViews1784", function (event) {
                                             modal.style.display = "none";
                                             hidePleaseWait();
                                             returnFunction("");
                                         });
        ...
        if (this.assessmentschedules != undefined && this.assessmentschedules.length == 0) {
            ...
            coController.coGetAssessmentSchedules(this.courseid, this.basemoduleid, function(ret) {
                                                  ...
                                                  that.PayToBookAction= ret.PayToBookAction;
                                                      ...
                                                      that.PayToBookPrice = ret.PayToBookAmount;
    ...
    try {
        ...
        coController.coAssessmentBookingCreate(this.courseid, this.basemoduleid, assessmentScheduleId, assessmentItemId, function(ret) {
                                               ...
                                               msgBtnValue = resources.btnOk;
                                                   ...
                                                   $("#boxTitle").html(msgTitle);
                                                   $("#box-string").html(msgStr);
                                                   $("#addbutton").html("<div id='assessmentBookingCreate1930' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                                   modal.style.display = "block";
                                                   $(document).off("vclick", "#assessmentBookingCreate1930");
                                                   $(document).on("vclick", "#assessmentBookingCreate1930", function (event) {
                                                       modal.style.display = "none";
                                                       hidePleaseWait();
                                                       returnFunction(false);
                                                   });
    ...
    try {
        ...
        coController.coAssessmentBookingDelete(this.courseid, this.basemoduleid, assessmentScheduleId, assessmentItemId, bookingId, function(ret) {
                                               ...
                                               msgBtnValue = resources.btnOk;
                                                   ...
                                                   $("#boxTitle").html(msgTitle);
                                                   $("#box-string").html(msgStr);
                                                   $("#addbutton").html("<div id='assessmentBookingDelete1984' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                                   modal.style.display = "block";
                                                   $(document).off("vclick", "#assessmentBookingDelete1984");
                                                   $(document).on("vclick", "#assessmentBookingDelete1984", function (event) {
                                                       modal.style.display = "none";
                                                       hidePleaseWait();
                                                       returnFunction(false);
                                                   });
...
/*getAssessmentPackageData: function(courseid,ACID,returnFunction){
    try{
        var TAPs_details = "";
        getassessmentPackage(false,courseid,ACID,function(ret){
              ...
              for(var i = 0;i<ret.packageListField.length;i++){
                  TAPs_details += '<div class="box-body ng-scope"><div> <div >';
        ...
        errorHandler("Module.getAssessmentPackageData", e);
    }
},*/
/*getAssessmentCenterData: function(courseid,ACID,returnFunction){
    try{
        var TAPs_details = "<option value=''>Select an Assessment Center:</option><option value='"+courseid+"-1'>Select an Assessment Center:</option><option value='"+courseid+"-2'>Select an Assessment Center:</option>";
        getassessmentPackage(false,courseid,ACID,function(ret){
           //$("#course_Name").empty();
           if(ret.accessmentCentersField.length > 0){
                $("#assessment_center-button").css("display","block");
                for(var i = 0;i<ret.accessmentCentersField.length;i++){
                   TAPs_details += '<option value="'+ret.accessmentCentersField[i].ACID+'">'+ret.accessmentCentersField[i].ACName+'</option>';
                }
           }
           else{
        ...
        errorHandler("Module.getAssessmentCenterData", e);
    }
},*/
    ...
    try {
        ...
        if (this.assessmentschedules == undefined || this.assessmentschedules.length == 0) {
            ...
            if (this.hasbooking) {
                ...
                var MobNo = this.PayToBookPhoneNumber;
                var PayToBookAmount = this.PayToBookPrice;
                var CalltoPay= MobNo.replace(/ /g,"");
                ...
                for (var i = 0; i < this.assessmentschedules.length; i++) {
                  if(GetAssessmentSchedulesParse.PayToBook == true && GetAssessmentSchedulesParse.CourseOverridePayToBook == false && GetAssessmentSchedulesParse.ModulePayToBookStatusID == 2){
                    ...
                    //schedulesStr += this.title + ", " + schedule.starttime + ", " + schedule.venuename;
                    var title = "";
                    if(schedule.venuename.length >= 14){
                        title = schedule.venuename.substring(0, 10) + "...";
                    }else{
                        title = schedule.venuename;
                    }
                    ...
                    schedulesStr += schedule.restrictbooking===true ? 'restrictbooking' : '';
                    schedulesStr += " ui-btn ui-btn-h' data-theme='h' href='#' id='book-" + schedule.assessmentscheduleid + "-" + schedule.assessmentitemid + "'>" + resources.bookbtntxtPayToBook + "</a></div>";
                    schedulesStr += "<div class='viewbookbtn' style='float:right !important;width: auto !important;padding-left: 5px;padding-right: 5px;'><a class='viewschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='viewbook-" + schedule.assessmentscheduleid + "-" + schedule.assessmentitemid + "'>" + resources.detailsbtntxt + "</a></div>";
                    schedulesStr += "</li>";
                  }
                  ...
                  //else if(GetAssessmentSchedulesParse.CourseOverridePayToBook == true){
                    ...
                    var schedulesId = "schedules-" + $.trim(highLightDates[(hd - 1)]);
                    schedulesStr += "<li class='scheduleitem  " + schedulesId + "' data-theme='h'><div class='scheduleitemmain'>";
                    schedulesStr += "<div style='width: auto !important;' class='scheduleitemhead'>";
                    //schedulesStr += this.title + ", " + schedule.starttime + ", " + schedule.venuename;
                    var title = "";
                    if(schedule.venuename.length >= 14){
                        title = schedule.venuename.substring(0, 10) + "...";
                    ...
                    schedulesStr += schedule.starttime + ", " + title;
                    schedulesStr += "</div>";
                    schedulesStr += "<div class='bookbtn' style='background-color:#55c7a6;padding-top: 7.1px;float:right !important;width: auto;padding-left: 5px;padding-right: 5px;'><a class='schedulebookbtn wthMsg";
                    ...
                    schedulesStr += schedule.starttime + ", " + title;
                    schedulesStr += "</div>";
                    schedulesStr += "<div class='bookbtn' style='background-color:#55c7a6;padding-top: 7.1px;float:right !important;width: auto;padding-left: 5px;padding-right: 5px;'><a class='schedulebookbtn ";
    ...
    try {
        ...
        for (var y = 0; y < this.assessmentschedules.length; y++) {
            ...
            if (scheduleBook.booking != undefined && scheduleBook.booking.assessmentbookingid > 0) {
                ...
                schedulesStr += "<div style='width:auto;' class='scheduleitemhead "+lesswidth+"'>";
                var title = "";
                if(scheduleBook.venuename.length >= 14){
                    title = scheduleBook.venuename.substring(0, 10) + "...";
                }else{
                    title = scheduleBook.venuename;
                }
                schedulesStr += scheduleBook.starttime + ", " + title;
                ...
                if (scheduleBook.booking.resultpending === false) {
                    ...
                    if (scheduleBook.booking.allowcancel == true) {
                        schedulesStr += "<div class='cancelbookbtn' style='float:right !important;padding-top: 12px;margin-left: 0px !important;width: auto;padding-left: 5px;padding-right: 5px;background-color: #A0B6CD !important;'><a style='background-color: #A0B6CD !important;' class='cancelschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='cancelbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "-" + scheduleBook.assessmentbookingid + "'>" + resources.cancelbtntxt + "</a></div>";
                        schedulesStr += "<div class='viewbookbtn' style='width: auto;padding-left: 5px;padding-right: 5px;float:right !important;'><a  class='viewschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='viewbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "'>" + resources.detailsbtntxt + "</a></div>";
                    } else {
                        if(scheduleBook.showattendbutton == true){
                            ...
                            schedulesStr += "<div class='cancelbookbtn' style='background-color: #55c7a6 !important;padding-top: 11px;width:54px;'><a style='background-color: #55c7a6 !important;' class='attendschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='"+scheduleBook.zoomjoinURL+"'>" + resources.AttendBtn + "</a></div>";/* zoommeetingpassword*/
                            showPassowrd = "NotPassowrd";
                            showPassowrdStar = "NotStar";
                        }
                        else{
                            schedulesStr += "<div class='cancelbookbtn' style='float:right !important;padding-top: 12px;margin-left: 0px !important;width: auto;padding-left: 5px;padding-right: 5px;background-color: #A0B6CD !important;'><a style='background-color: #A0B6CD !important;' class='nocancelschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='cancelbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "-" + scheduleBook.assessmentbookingid + "'>" + resources.cancelbtntxt + "</a></div>";
                            schedulesStr += "<div class='viewbookbtn' style='width: auto;padding-left: 5px;padding-right: 5px;float:right !important;'><a  class='viewschedulebookbtn ui-btn ui-btn-h' data-theme='h' href='#' id='viewbook-" + scheduleBook.assessmentscheduleid + "-" + scheduleBook.assessmentitemid + "'>" + resources.detailsbtntxt + "</a></div>";
                        }
                    ...
                    schedulesStr += "<div style='width:117px !important;text-align:center;float: right;margin-left: 0% !important;padding-top: 11px;' class='cancelbookbtn greyColorButton'>"+ resources.bookingAwaitingResult +"</div></li>";
...
var TheoryModule = function(courseid, basemoduleid, haspodcast, pdfurl, podcasturl, LSURLIsYUDU, YUDUID,BuyPackage) {
    try {
        this.init(courseid, basemoduleid, haspodcast, pdfurl, podcasturl, LSURLIsYUDU, YUDUID,BuyPackage);
    } catch (e) {
        errorHandler("TheoryModule", e);
    }
};
TheoryModule.prototype = {
init: function(courseid, basemoduleid, haspodcast, pdfurl, podcasturl, LSURLIsYUDU, YUDUID,BuyPackage) {
    try {
        ...
        if (this.worksheet.questions.length == 0) {
            ...
            coController.coGetWorksheetData(this.theorymodulesectionid, function(ret) {
                                            ...
                                            msgBtnValue = resources.btnOk;
                                                ...
                                                $("#boxTitle").html(msgTitle);
                                                $("#box-string").html(msgStr);
                                                $("#addbutton").html("<div id='getWorksheetData2739' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                                modal.style.display = "block";
                                                $(document).off("vclick", "#getWorksheetData2739");
                                                $(document).on("vclick", "#getWorksheetData2739", function (event) {
                                                    modal.style.display = "none";
                                                    hidePleaseWait();
...
getSCOLi: function() {
    try {
        ...
        //alert(title);
        var titleclass="";
        if(title.length < 51 && tablet===false && iphone5===true){
            titleclass=" scosingleline";
        }else if(title.length < 51 && tablet===false && iphone5==false){
            titleclass=" scosingleline";
        }
    ...
    try {
        ...
        coController.coSaveWorksheetData(returnWorksheet, function(ret) {
                                         ...
                                         msgBtnValue = resources.btnOk;
                                             ...
                                             $("#boxTitle").html(msgTitle);
                                             $("#box-string").html(msgStr);
                                             $("#addbutton").html("<div id='coSaveWorksheetData3362' class='closeTAPs' style='padding: 10px;text-align: center;background-color: #55c7a6 !important;color: black;margin: 11px;width: 17%;margin-left: 36%;' data-dismiss='modal'>" + msgBtnValue + "</div>");

                                             modal.style.display = "block";
                                             $(document).off("vclick", "#coSaveWorksheetData3362");
                                             $(document).on("vclick", "#coSaveWorksheetData3362", function (event) {
                                                 modal.style.display = "none";
                                                 hidePleaseWait();
                                         ...
                                         that.result = ret.Result;
                                            window.localStorage.removeItem("isThirdWorksheetCompleted");
